--[[
-- EDITED BY GEN.WILHELM KEITEL, KINETHIC, AND KARMACREEPER
Zombie Survival
...
function GM:SetupProps()
	for _, ent in pairs(ents.FindByClass("prop_physics*")) do
		...
		if mdl then
			...
			if mdl == "models/props_c17/lockers001a.mdl" then
				local phys = ent:GetPhysicsObject()
				if phys:IsValid() then
					phys:SetMass(500)
				end
			end
			if mdl == "models/props_wasteland/controlroom_storagecloset001a.mdl" then
				local phys = ent:GetPhysicsObject()
				if phys:IsValid() then
					phys:SetMass(500)
				end 
			end
			if mdl == "models/props/cs_militia/refrigerator01.mdl" then
				local phys = ent:GetPhysicsObject()
				if phys:IsValid() then
					phys:SetMass(550)
				end
			...
			if table.HasValue(fuckedProps, mdl) then
				local phys = ent:GetPhysicsObject()
				if phys:IsValid() then
					ent:Remove()
				end
			end
			--Had to do it that way, I'm afraid to try the table method, but I may make it that way in future
...
--[[function GM:ShouldRestartRound()
	...
	if self.TimeLimit == 0 and self.RoundLimit == 0 then return false end
	...
	if NextTick <= time then
		...
		for _, pl in pairs(allplayers) do
			if P_Team(pl) == TEAM_HUMAN and P_Alive(pl) then
				...
				if pl:IsSkillActive(SKILL_REGENERATOR) and time >= pl.NextRegenerate and pl:Health() < math.min(healmax, pl:GetMaxHealth() * 0.75) then
				...
				if pl:KeyDown(IN_SPEED) and pl:GetVelocity() ~= vector_origin and pl:IsSkillActive(SKILL_CARDIOTONIC) then
					if pl:GetBloodArmor() > 0 then
						pl:SetBloodArmor(pl:GetBloodArmor() - 2)
				...
				if --[[pl:IsSkillActive(SKILL_STOWAGE) and]] self:GetWave() > 0 and time > (pl.NextResupplyUse or 0) then
					...
					pl.NextResupplyUse = time + self.ResupplyBoxCooldown * (pl.ResupplyDelayMul or 1) * (stockpiling and 1 or 1)
	...
	elseif self:GetNumberOfWaves() == -1 or self.NoNewHumansWave <= self:GetWave() or team.NumPlayers(TEAM_UNDEAD) == 0 and 1 <= team.NumPlayers(TEAM_HUMAN) then -- Joined during game, no zombies, some humans or joined past the deadline.
		...
		if self.DynamicSpawning then
			timer.Simple(0, function()
				GAMEMODE:AttemptHumanDynamicSpawn(pl)
				pl:SetBarricadeGhosting(false, false)
			end)
		end
	end
...
function GM:PropBroken(ent, attacker)
	if IsValid(ent) and IsValid(attacker) and not ent._PROPBROKEN and attacker:IsPlayer() and attacker:Team() == TEAM_HUMAN then
		...
		if attacker.LogID  and attacker.LogID ~= nil  then --failsafe for local dev
...
function GM:NestDestroyed(ent, attacker)
	if IsValid(ent) and IsValid(attacker) and attacker:IsPlayer() and attacker:Team()  and attacker.LogID ~= nil  == TEAM_UNDEAD then
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	if inflictor:IsValid() and inflictor == attacker:GetActiveWeapon() then
		...
		attacker:AddZSPrestige(inflictor, 1)
	end
...
VoiceSetTranslate["models/player/aileri_jaina.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/fursonas/krystalfox2k17spm.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/player/mvsc3_felicia.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/player/alicede_sharkfurry.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/palutena/palutena_body.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/mgs4/meryl.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/mgs4/meryl_servicedress.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/mailer/characters/valeera.mdl"] = VOICESET_FEMALE
VoiceSetTranslate["models/jazzmcfly/rwby/yang_xiao_long.mdl"] = VOICESET_FEMALE
function GM:PlayerSpawn(pl)
	...
	if pl:GetModel() == "models/player/mvsc3_felicia.mdl" then
		pl.SkillSpeedAdd = 50
	end
